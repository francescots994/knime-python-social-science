...
class SarimaLearner:
    ...
    *Seasonal versions of these components operate similarly, with lag intervals equal to the seasonal period (S).*
    ...
    def configure(
        ...
        # handle P > 0 and p >= s
        if (self.min_s_ar > 0) and (self.min_ar >= self.seasonal_period_param):
            configure_context.set_warning(
        ...
        # handle Q > 0 and q >= s
        if (self.min_s_ma > 0) and (self.min_ma >= self.seasonal_period_param):
            configure_context.set_warning(
        ...
        if (
            self.max_ar < self.min_ar
            or self.max_i < self.min_i
            or self.max_ma < self.min_ma
            or self.max_s_ar < self.min_s_ar
            or self.max_s_i < self.min_s_i
            or self.max_s_ma < self.min_s_ma
        ):
            configure_context.set_warning(
        ...
        binary_model_schema = knext.BinaryPortObjectSpec(
            "auto_sarima.model"
        )  # change ID for model
    ...
    def execute(self, exec_context: knext.ExecutionContext, input: knext.Table):
        ...
        if (
            (self.min_ar == self.max_ar)
            and (self.min_i == self.max_i)
            and (self.min_ma == self.max_ma)
            and (self.min_s_ar == self.max_s_ar)
            and (self.min_s_i == self.max_s_i)
            and (self.min_s_ma == self.max_s_ma)
        ):
        ...
        preds_col = trained_model.predict(start=1, dynamic=self.dynamic_check)
    ...
    def __validate(self, column):
        ...
        set_val = set(
            [
                ...
                self.ma_order_param,
                # S * P
                self.seasonal_period_param * self.seasonal_ar_order_param,
                # S*Q
                self.seasonal_period_param * self.seasonal_ma_order_param,
            ]
        )
    ...
    def diff_parameters(
        self, series, seasonality, max_i, min_i, max_is, min_is, alpha=0.05
    ):
        """
        ...
        if p_value_ds > alpha:
            ds_parameter = min(
                max(0, min_is), max_is
            )  # Series is likely stationary at seasonal level (Fail to reject H₀)."
            ...
            if p_value_d > alpha:
                ...
                if p_value_d > alpha:
                    print(
                        "Series is stationary at seasonal level, but needs difference at trend level (d=1, D=0)."
                    )
                    return (min(max(1, min_i), max_i), ds_parameter)
                else:
                    print(
                        "Series is stationary at seasonal level, but needs 2 differences at trend level (d=2, D=0)."
                    )
            ...
            ds_parameter = min(
                max(1, min_is), max_is
            )  # Series is likely non stationary at seasonal level (Reject H₀, differencing needed)
            ...
            p_value_d = kpss(first_seasonal_diff_series, regression="c", nlags="auto")[
                1
            ]

            if p_value_d > alpha:
                print(
                    "Series is non stationary at seasonal level and stationary at trend level (d=0, D=1)."
                )
                return (min(max(0, min_i), max_i), ds_parameter)
            else:
                ...
                if p_value_d > alpha:
                    print(
                        "Series is non stationary at seasonal level and needs difference at trend level (d=1, D=1)."
                    )
                    return (min(max(1, min_i), max_i), ds_parameter)
                else:
                    print(
                        "Series is non stationary at seasonal level and needs 2 differences at trend level (d=2, D=1)."
                    )
                    return (min(max(2, min_i), max_i), ds_parameter)

    def propose_initial_params(
        self,
        d,
        D,
        min_p=0,
        ...
        try:
            ...
            model_fit = SARIMAX(
                ...
                order=(
                    ...
                    params["d"],
                    params["q"],
                ),  # p, d, q order parameters for arima
    ...
    def simann(
        ...
        d, D = self.diff_parameters(
            series, seasonality, max_i, min_i, max_is, min_is, alpha=0.05
        )
        current_params = self.propose_initial_params(
            d, D, min_p, max_p, min_q, max_q, min_ps, max_ps, min_qs, max_qs
        )
        ...
        for beta in beta_list:
            ...
            for t in range(mcmc_steps):
                ...
                delta_c = (
                    self.evaluate_arima_model(series, move, seasonality) - current_c
                )
